...
Sbox = np.array(Sbox)   # using array in order to call Sbox[np.ndarray]
...
InvSbox = np.array(InvSbox)    # using array in order to call InvSbox[np.ndarray]
...
def clean(tr: np.ndarray, _b, _a):
    ...
    for i in range(len(tr)):
        tr[i] = signal.lfilter(_b, _a, tr[i])
    return tr
